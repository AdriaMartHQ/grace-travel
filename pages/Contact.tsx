  ...
  const handleCopyWeChat = async () => {
    try {
      if (navigator.clipboard && navigator.clipboard.writeText) {
        await navigator.clipboard.writeText(weChatID.replace(/\s/g, ''));
        setCopied(true);
      ...
      const msg =
        language === 'zh'
          ? '复制失败，请长按号码手动复制。'
          : language === 'tr'
          ? 'Kopyalama başarısız, lütfen numarayı uzun basarak kopyalayın.'
          : 'Copy failed. Please long-press the number to copy.';
      window.alert(msg);
    }
  };
