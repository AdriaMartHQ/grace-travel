...
const ItineraryZ5: React.FC = () => {
  const { t, language } = useLanguage();
  const d = (t as any).itinerary_z5 || {};
  const currencySymbol = '¥';

  const fallbackImage =
    'https://images.unsplash.com/photo-1605382628707-0aa0593fba19?auto=format&fit=crop&w=1600&q=80';
        ...
        <div className="text-center space-y-6 max-w-sm">
          ...
          <p className="text-slate-500 font-bold uppercase tracking-widest text-xs leading-relaxed">
            {language === 'zh'
              ? '正在为您加载 Z5 信仰之旅行程...'
              : language === 'tr'
              ? 'Z5 inanç yolculuğu yükleniyor...'
              : 'Loading Z5 faith journey...'}
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="bg-[#fdfcfb] text-slate-900 selection:bg-orange-100 overflow-x-hidden">
      <SEO
        title={d.seo?.title || d.meta?.title}
        description={d.seo?.description || d.meta?.subtitle}
      ...
      <section className="relative h-[80vh] md:h-[85vh] flex items-center justify-center text-center px-4 overflow-hidden">
        <div className="absolute inset-0 z-0 scale-110">
          <img
            src="https://images.unsplash.com/photo-1605382628707-0aa0593fba19?q=80&w=2340&auto=format&fit=crop"
            className="w-full h-full object-cover"
            alt="Grace Way Turkey Z5"
        ...
        <div className="relative z-20 max-w-6xl animate-in fade-in slide-in-from-bottom-6 duration-1000">
          <span className="inline-block px-3 py-1 bg-[#FF9D00] text-white text-[9px] md:text-[10px] font-black uppercase tracking-[0.18em] md:tracking-[0.3em] rounded mb-6">
            {d.meta?.code || 'Z5'} Series · Faith Path
          </span>
          ...
          <div className="max-w-7xl mx-auto px-6 relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
              {(d.highlights?.items || []).map((item: { title: string; desc: string }, i: number) => (
                <div
                  key={i}
                  className="p-8 border border-white/10 rounded-[2rem] bg-white/5 backdrop-blur-sm flex gap-6 items-start"
                >
      ...
      {d.summaryTable && (
        <section className="py-20 max-w-7xl mx-auto px-4 md:px-6">
          <div className="text-center mb-10">
            <h2 className="text-2xl md:text-3xl font-black serif uppercase tracking-widest mb-2">
              {d.summaryTable?.title}
            </h2>
          </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {(d.summaryTable?.rows || []).map((row: any, i: number) => (
                  <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    {row.breakfast !== undefined && (
                      <td className="px-6 py-5 text-center text-[11px] text-slate-500 font-medium">
                        {row.breakfast}
                      </td>
                    )}
                    {row.lunch !== undefined && (
      ...
      <section className="py-20 md:py-32 space-y-24 md:space-y-40">
        <div className="max-w-7xl mx-auto px-6 mb-20">
          <h2 className="text-3xl md:text-5xl font-black serif text-center italic">
            {language === 'zh' ? '行程安排详解' : language === 'tr' ? 'Detaylı Program' : 'Detailed Itinerary'}
          </h2>
          <div className="w-16 h-1 bg-[#FF9D00] mx-auto mt-4"></div>
        </div>
          ...
          >
            ...
            <div className="w-full lg:w-1/2 space-y-6 text-left">
              ...
              <div className="p-6 md:p-10 bg-[#fdfaf6] rounded-[2rem] border border-orange-100/50 flex flex-col sm:flex-row gap-6 md:gap-12 mt-8">
                <div>
                  <p className="text-[9px] font-black text-orange-300 uppercase tracking-widest mb-1">
                    {d.meta?.meals_label}
                  </p>
                  <p className="text-slate-800 font-bold text-xs">
                    {language === 'zh' ? '早' : 'B'}：{item.meals?.breakfast} / {language === 'zh' ? '午' : 'L'}：
                    {item.meals?.lunch} / {language === 'zh' ? '晚' : 'D'}：{item.meals?.dinner}
                  </p>
                </div>
                <div>
                  <p className="text-[9px] font-black text-orange-300 uppercase tracking-widest mb-1">
